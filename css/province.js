...
for(j = 1;j < loca3.length;j++) { with(document.getElementById('city')) { length = loca3.length; options[j].text = loca3[j]; options[j].value = loca3[j];options[0].value="";options[0].text="请选择城市名"}}
break;
}
}
}
...
function SelectAlls(province,city) {
with(document.getElementById(province)) { var loca2 = options[selectedIndex].value; }
for(i = 0;i < where.length;i ++) {
if (where[i].loca == loca2) {
loca3 = (where[i].locacity).split("|");
